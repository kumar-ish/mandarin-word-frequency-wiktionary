...
for lowerIndex in ${freqRanges[@]}; do
    ...
    wget -O $rangeName.buffer.txt "https://en.wiktionary.org/w/api.php?action=query&prop=revisions&titles=Appendix:Mandarin_Frequency_lists/${rangeName}&rvslots=*&rvprop=content&formatversion=2&format=json" && cat $rangeName.buffer.txt | jq -r '.query.pages[0].revisions[0].slots.main.content' | sed '$d' | tail -n +2 | tee $rangeName.txt >> words.txt
